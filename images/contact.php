...
<?php
  if( isset($_POST['contact_name']) ) {
    if( empty($_POST['contact_message']) ) {
     echo '<span class="help-block"><code class="text-info">Please do not forget to type in your message before submitting.</code></span>';
    }
    else {
     $name = htmlspecialchars($_POST['contact_name']);
     $email = htmlspecialchars($_POST['contact_name']);
     $phone = (htmlspecialchars($_POST['contact_name'])) ? htmlspecialchars($_POST['contact_name']) : '';
     $message = 'Name: '.$name.'<br />E-mail: '.$email.'<br />Phone: '.$phone.'<br />Message: '.htmlspecialchars($_POST['contact_message']);
